 ...
 *
 * <p> A convenience implementation of a select input that contains
 * the 12 months of the Gregorian calender. </p>
 */
public class JQMMonth extends JQMSelect {
    ...
    public void setMonthNames(String[] months) {
        int cnt = getOptionCount();
        for (int i = 0; i < cnt; i++) {
            OptionElement op = getOption(i);
            op.setText(months[i]);
        }
        refresh();
    }
